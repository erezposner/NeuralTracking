...
class BackprojectFisheye(Backproject):
    def __init__(self, batch_size, height, width, device):
        super(BackprojectFisheye, self).__init__(batch_size, height, width, device)

    def forward(self, depth, inv_K):
        # depth is radial - it is converted to z depth
        # cam_points can be caluclated ones in init if K is constant for all images
        cam_points = torch.matmul(inv_K[:, :3, :3], self.pix_coords)

        r = torch.sqrt(cam_points[:, 0, :]**2 + cam_points[:, 1, :]**2)
        r = r.unsqueeze(1) + 1e-7
        cam_points[:, :2, :] = (cam_points[:, :2, :]/r)*torch.tan(r)

        bearing_vectors = (cam_points/torch.norm(cam_points, dim=1, keepdim=True,p=2))
        rays = depth.view(self.batch_size, 1, -1)  * bearing_vectors
        depth = rays[:,2, :].unsqueeze(1)
        xy = depth * cam_points
...
class ProjectFisheye(Project):
    def __init__(self, batch_size, height, width, device, eps=1e-7):
        super(ProjectFisheye, self).__init__(batch_size, height, width, eps)
        self.device = device
        self.ones = torch.ones(self.batch_size, 1, self.height * self.width)
        self.ones = self.ones.to(self.device)
    ...
    def forward(self, points, K, T):
        # P = torch.matmul(K, T)[:, :3, :]
        points_transformed = torch.matmul(T, points)
        points_transformed = points_transformed[:,:3, :]
        ab = points_transformed/points_transformed[:, 2, :].unsqueeze(1)
        r = torch.sqrt((ab[:, :2, :]**2).sum(axis=1))
        r = r.unsqueeze(1)
        theta = torch.atan(r)
        x_y_ = theta*(ab[:, :2, :]/r)
        x_y_ = torch.cat([x_y_, self.ones], 1)

        # cam_points = torch.matmul(P, x_y_)
        cam_points = torch.matmul(K[:, :3, :3], x_y_)
        pix_coords = cam_points[:, :2, :] / (cam_points[:, 2, :].unsqueeze(1) + self.eps)
        pix_coords = pix_coords.view(self.batch_size, 2, self.height, self.width)
        pix_coords = pix_coords.permute(0, 2, 3, 1)
        pix_coords[..., 0] /= self.width - 1
