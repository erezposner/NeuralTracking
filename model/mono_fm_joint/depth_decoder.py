...
class DepthDecoder(nn.Module):
    def __init__(self,  num_ch_enc):
        ...
        #depth - rename if you want to reload weights
        self.depth4 = nn.Sequential(Conv3x3(bottleneck, 1), nn.Softplus())
        self.depth3 = nn.Sequential(Conv3x3(bottleneck, 1), nn.Softplus())
        self.depth2 = nn.Sequential(Conv3x3(bottleneck, 1), nn.Softplus())
        self.depth1 = nn.Sequential(Conv3x3(bottleneck, 1), nn.Softplus())
    ...
    def forward(self, input_features, frame_id=0):
        ...
        x4 = upsample(x4)
        # disp4 = self.disp4(x4)
        depth4  = self.depth4(x4)

        x3 = self.reduce3(l3)
        x3 = torch.cat((x3, x4, depth4), 1)
        x3 = self.iconv3(x3)
        x3 = F.leaky_relu(x3)
        x3 = self.crp3(x3)
        x3 = self.merge3(x3)
        ...
        x3 = upsample(x3)
        # disp3 = self.disp3(x3)
        depth3 = self.depth3(x3)

        x2 = self.reduce2(l2)
        x2 = torch.cat((x2, x3 , depth3), 1)
        x2 = self.iconv2(x2)
        x2 = F.leaky_relu(x2)
        x2 = self.crp2(x2)
        x2 = self.merge2(x2)
        ...
        x2 = upsample(x2)
        # disp2 = self.disp2(x2)
        depth2 = self.depth2(x2)

        x1 = self.reduce1(l1)
        x1 = torch.cat((x1, x2, depth2), 1)
        x1 = self.iconv1(x1)
        x1 = F.leaky_relu(x1)
        x1 = self.crp1(x1)
        x1 = self.merge1(x1)
        ...
        x1 = upsample(x1)
        # disp1 = self.disp1(x1)
        depth1 = self.depth1(x1)

        self.outputs[("depth", frame_id, 3)] = depth4 #disp4
        self.outputs[("depth", frame_id, 2)] = depth3 #disp3
        self.outputs[("depth", frame_id, 1)] = depth2#disp2
        self.outputs[("depth", frame_id, 0)] = depth1 #disp1
